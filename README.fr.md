...
│  - LlmAgent (Claude/Gemini, interchangeable)│
...
- **Le modèle de raisonnement est interchangeable.** Claude Sonnet sur Vertex AI par défaut ; il peut être remplacé par Gemini ou l'API publique d'Anthropic via une simple variable de configuration, sans toucher au pipeline de rendu (voir ci-dessous).
...
- Pour le fournisseur par défaut (`claude`) : le **modèle Claude activé dans Vertex AI Model Garden** (le script le vérifie dans une étape de préflight).
...
Modifiez les variables en haut de `gemini_looker_v1.sh` avant de l'exécuter :
...
AGENT_DESCRIPTION="Agent Looker qui affiche les dashboards en images intégrées et répond aux questions de données."

# Modèle de raisonnement (voir la section ci-dessous)
AGENT_MODEL_PROVIDER="claude"  # "claude", "claude_native", "anthropic" ou "gemini"
CLAUDE_MODEL="claude-sonnet-4-6"
CLAUDE_LOCATION="us-east5"     # région Vertex qui sert Claude
...
4. Préflight : vérifier que le modèle Claude choisi est activé dans Model Garden (ignoré pour `gemini`/`anthropic`)
5. Créer le compte de service de l'agent et accorder un IAM minimal
6. Créer le bucket de staging (déploiement + artifacts)
7. Préparer un environnement Python isolé
8. Construire l'application de l'agent (ADK)
9. Déployer l'agent sur Agent Engine *(~15–20 min)*
10. Enregistrer l'agent dans Gemini Enterprise
...
- `show_dashboard_inline` — rend un dashboard en PNG, éventuellement pré-filtré via `filters`
...
- `get_dashboard_link`, `get_look_link`, `get_explore_link` — renvoient une URL d'embed SSO signée (`get_dashboard_link` accepte des `filters` optionnels)
...
- `list_looker_explores` — liste les explores d'un modèle
...
- `list_dashboard_filters` — liste les filtres d'un dashboard (noms et valeurs)

---

## Changer le modèle de raisonnement

Le modèle se choisit via des variables de configuration — aucune modification de code. `litellm` et `anthropic[vertex]` sont déjà inclus dans les requirements du déploiement.
...
- **`claude`** *(par défaut)* — Claude sur Vertex AI via LiteLLM. Pas de clé API ; facturé via GCP. Activez le modèle dans **Vertex AI Model Garden** et définissez `CLAUDE_LOCATION` sur une région qui le sert (le script le vérifie dans une étape de préflight).
- **`claude_native`** — Claude sur Vertex AI via le wrapper natif de l'ADK (un chemin de code différent de LiteLLM ; essayez-le si le streaming fonctionne mais que Gemini Enterprise renvoie des réponses vides).
- **`anthropic`** — Claude via l'API publique d'Anthropic ; nécessite `ANTHROPIC_API_KEY`.
- **`gemini`** — Gemini sur Vertex AI (`GEMINI_MODEL`).

Le modèle de raisonnement est indépendant du rendu : changer de fournisseur n'affecte jamais la fonctionnalité d'images.
