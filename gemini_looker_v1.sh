...
#
# Delegamos la firma a Looker via create_sso_embed_url en vez de firmar a mano.
# Razon: el firmado manual (HMAC) es muy fragil -> hay que acertar el endpoint
# exacto (/login/embed/<ruta codificada>), el orden EXACTO de los campos en el
# string a firmar, y el algoritmo correcto (los secretos creados por UI usan
# SHA-1; los creados por API usan SHA-256). Cualquier desajuste = 401. El SDK
# arma todo eso correctamente por nosotros.
#
# Requisitos en Looker para que esto funcione:
#   - Admin > Embed: "Embed SSO Authentication" = Enabled (+ Reset Secret).
#   - El usuario API debe tener permiso admin/embed.
#   - El dominio desde el que se abre el link debe estar en el allowlist de embed.
#
# Nota: create_sso_embed_url espera la URL NORMAL de la UI (sin /embed). Los
# callers pasan "/embed/dashboards/1"; aqui quitamos el prefijo "/embed".
# -----------------------------------------------------------------------------
def _generate_signed_embed_url(target_path: str, user_email: str = "gemini-user") -> str:
    ui_path = target_path[len("/embed"):] if target_path.startswith("/embed") else target_path
    sdk = looker_sdk.init40()
    result = sdk.create_sso_embed_url(
        body=models40.EmbedSsoParams(
            target_url=f"https://{LOOKER_HOST}{ui_path}",
            session_length=3600,
